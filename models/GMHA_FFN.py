...
class MTL_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        # Gating network
        self.gating_network = nn.Linear(embedding_dim, 4)  # 4 gates for 4 different attention mechanisms
    ...
    def forward(self, depot_xy, node_xy_demand_tw):
        ...
        for layer in self.layers:
            out = layer(out, self.gating_network)
...
class EncoderLayer(nn.Module):
    def __init__(self, **model_params):
        ...
        self.Wk_1 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.Wv_1 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        ...
        self.Wk_3 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.Wv_3 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)

        self.Wq_4 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.Wk_4 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.Wv_4 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
    ...
    def forward(self, input1, gating_network):
        """
        Two implementations:
        ...
        # Gating mechanism
        gate_scores = gating_network(input1)
        gate_probs = F.softmax(gate_scores, dim=-1)

        # Attention mechanism 1
        q1 = reshape_by_heads(self.Wq_1(input1), head_num=head_num)
        k1 = reshape_by_heads(self.Wk_1(input1), head_num=head_num)
        v1 = reshape_by_heads(self.Wv_1(input1), head_num=head_num)
        attn_out_1 = multi_head_attention(q1, k1, v1)

        # Attention mechanism 2
        q2 = reshape_by_heads(self.Wq_2(input1), head_num=head_num)
        k2 = reshape_by_heads(self.Wk_2(input1), head_num=head_num)
        v2 = reshape_by_heads(self.Wv_2(input1), head_num=head_num)
        attn_out_2 = multi_head_attention(q2, k2, v2)

        # Attention mechanism 3
        q3 = reshape_by_heads(self.Wq_3(input1), head_num=head_num)
        k3 = reshape_by_heads(self.Wk_3(input1), head_num=head_num)
        v3 = reshape_by_heads(self.Wv_3(input1), head_num=head_num)
        attn_out_3 = multi_head_attention(q3, k3, v3)

        # Attention mechanism 4
        q4 = reshape_by_heads(self.Wq_4(input1), head_num=head_num)
        k4 = reshape_by_heads(self.Wk_4(input1), head_num=head_num)
        v4 = reshape_by_heads(self.Wv_4(input1), head_num=head_num)
        attn_out_4 = multi_head_attention(q4, k4, v4)

        # Combining the outputs using gate probabilities
        multi_head_out = (gate_probs[:, :, 0].unsqueeze(-1) * attn_out_1 +
                          gate_probs[:, :, 1].unsqueeze(-1) * attn_out_2 +
                          gate_probs[:, :, 2].unsqueeze(-1) * attn_out_3 +
                          gate_probs[:, :, 3].unsqueeze(-1) * attn_out_4)

        multi_head_out = self.multi_head_combine(multi_head_out)  # (batch, problem, EMBEDDING_DIM)
